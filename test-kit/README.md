...
This test kit provides a complete Docker-based environment for testing and developing with Apache Flink 1.20.3, including:
...
```bash
docker compose exec pyflink python3 /app/simple_table_api.py
```

**Streaming Join Example:**
```bash
docker compose exec pyflink python3 /app/streaming_join.py temporal
```

**Generate Kafka Events:**
```bash
docker compose exec pyflink python3 /app/kafka_data_generator.py --rate 10
```
...
**Access shell:**
```bash
docker compose exec pyflink bash
```

**Run Python script:**
```bash
docker compose exec pyflink python3 /app/your_script.py
```
...
**Batch mode:**
```bash
docker compose exec pyflink python3 /app/simple_table_api.py batch
```
...
```bash
docker compose exec pyflink python3 /app/streaming_join.py aggregation
```

**Regular stream-stream join:**
```bash
docker compose exec pyflink python3 /app/streaming_join.py regular
```
...
```bash
docker compose exec pyflink python3 /app/kafka_data_generator.py --sample
```

**Continuous generation to Kafka:**
```bash
docker compose exec pyflink python3 /app/kafka_data_generator.py \
...
**Check logs:**
```bash
docker compose logs sql-gateway
```
...
- Use `kafka:29092` from within Docker network
...
**Check Java version:**
```bash
docker compose exec pyflink java -version
...
```bash
# 1000 events/second
docker compose exec pyflink python3 /app/kafka_data_generator.py --rate 1000

# Multiple producers in parallel
for i in {1..5}; do
  docker compose exec -d pyflink python3 /app/kafka_data_generator.py --rate 200
done
```
...
4. **Test PyFlink jobs** in pyflink container
5. **Monitor jobs** via Flink Web UI
