...
# Run PyFlink example
docker compose exec pyflink python3 /app/simple_table_api.py --sample
...
### Issue: Docker Desktop File Sharing

**Problem**: Docker Desktop for Mac wasn't recognizing newly created subdirectories for bind mounts.

**Solution**: Removed all bind mounts for SQL init scripts and PyFlink code. Instead:
- SQL scripts are copied into containers via `docker cp` in `initialize.sh`
- PyFlink scripts are copied into container via `docker cp`
